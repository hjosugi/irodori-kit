...
`irodori-extension` crate in this `irodori-kit` workspace through the same
...
node bin/irodori-extension-dev.mjs templates/typescript-basic
```
...
`tools/typegen.mjs` uses the parent `irodori-kit` workspace by default. Override
with `IRODORI_KIT=/path/to/irodori-kit` only when running the package from a
